...
The default partition scheme (1.2 MB APP + OTA) may not be large enough. In the Arduino IDE, select **Tools > Partition Scheme > "NO OTA (2MB APP/2MB SPIFFS)"**. If using PlatformIO, add `board_build.partitions = no_ota.csv` to `platformio.ini`.
